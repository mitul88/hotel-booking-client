  ...
  return (
    <div className='p-1 w-96 md:w-2/5'>
        <Form method='post'>
            ...
            <button className='px-5 py-2 rounded-md bg-orange-600 text-white font-semibold tracking-wider' disabled={isSubmitting}>{isSubmitting ? "Submitting" : isLogin ? 'Login' : 'Register'}</button>
        </Form>
        <div className="flex flex-row items-center mt-[50px]">
            ...
            <Link to={`?mode=${isLogin ? 'register' : 'login'}`} className='mx-2 py-1 px-3 rounded-md border-2 border-gray-500 font-semibold'>
